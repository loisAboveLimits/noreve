...
<?php if ( $has_orders ) : ?>
	...
	<div id="order-lists" class="">
		<!-- <h1 class="section-title fnt-gold remMar"><?php pll_e('Other Orders'); ?></h1>
		<p class="section-subtitle remMar"><?php pll_e('Other Orders'); ?></p> -->
				...
				<div class="row align-items-start">
					...
					<div class="col-md-10">
						<div class="item-details">
							...
							<p class="fnt-20"><?php pll_e('Order Number'); ?> | <b><?php echo _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number();?></b></p>
							<p class="fnt-20">
								<b>
									<?php
							...
							<p class="fnt-14 fnt-gray">
								<?php pll_e('Order Date'); ?> <?php echo esc_html( wc_format_datetime( $order->get_date_created() ) ); ?>
							</p>

							<div class="product-items">


								<?php $items = getOrderItems($order->get_items()); ?>

								<?php foreach($items as $rowItem){ ?>

									<a href="<?php print $rowItem['link']; ?>" target="_new" data-item="<?php print $rowItem['id']; ?>">
										<div class="items">
											<div class="row align-items-start">
												...
												<div class="col-3">
													...
													<div class="product-img">
														<img src="<?php print $rowItem['image']; ?>" alt="product" class="img-fluid">
													</div>

												</div>
												<div class="col-9">

													<h5 class="fnt-20"><?php print $rowItem['title'];?></h5>
													<p class="fnt-14 fnt-light-gray"><?php print $rowItem['excerpt'];?></p>
					...
					<div class="col-md-2 text-center">
						...
						<?php  
							...
							$bgStat = $order->get_status();
							$bgColor = "";

							switch ($bgStat) {
							    case 'pending':
							        $bgColor = "background-color:rgba(245, 158, 11, 1);";
							        break;

							    case 'processing':
							        $bgColor = "background-color:rgba(59, 130, 246, 1);";
							        break;

							    case 'completed':
							        $bgColor = "background-color:rgba(34, 197, 94, 1);";
							        break;
							    ...
							    default:
							        $bgColor = "background-color:rgba(174, 153, 98, 1);";
						...
						<div class="order-status fnt-white" style="<?php print $bgColor;?>">
							...
							<?php
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										...
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button' . esc_attr( $wp_button_class ) . ' button ' . sanitize_html_class( $key ) . ' btn btn-sm btn-gernetic-black-square fnt-14 btn-stat" aria-label="' . esc_attr( $action_aria_label ) . '">' . esc_html( $action['name'] ) . '</a>
