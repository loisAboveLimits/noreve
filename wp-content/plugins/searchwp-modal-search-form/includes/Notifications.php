...
/**
 * Class SearchWPModalFormNotifications.
 *
...
class SearchWPModalFormNotifications {
	...
	public static function init() {
		...
		add_filter( 'searchwp_modal_form_settings_defaults', [ __CLASS__, 'hide_opt_out_setting' ] );
		...
		add_filter( 'searchwp_modal_form_options_submenu_pages', [ __CLASS__, 'add_submenu_page' ] );

		add_action( 'searchwp_modal_form_settings_header_actions', [ __CLASS__, 'output_header_button' ] );
		add_action( 'searchwp_modal_form_settings_header_after', [ __CLASS__, 'output_panel' ] );

		add_action( 'wp_ajax_searchwp_modal_form_notification_dismiss', [ __CLASS__, 'dismiss' ] );
    }
	...
	public static function has_access() {
		...
		return current_user_can( SettingsApi::get_capability() ) && ! self::is_disabled_in_settings();
	}

	/**
	 * Check if notifications are disabled in settings.
     * Uses Live Ajax Search setting if active.
	 *
	 * @since 0.5.1
	 *
	 * @return bool
	 */
	private static function is_disabled_in_settings() {

		if ( Utils::is_live_search_active() ) {
			$settings_api = searchwp_live_search()->get( 'Settings_Api' );
		} else {
			$settings_api = searchwp_modal_form()->get( 'SettingsApi' );
        }

		return $settings_api->get( 'hide-announcements' );
	}
	...
	public static function assets_global() {
		...
		wp_enqueue_style(
			'searchwp-modal-form-admin-notifications-global',
			SEARCHWP_MODAL_FORM_PLUGIN_URL . 'assets/styles/admin/notifications-global.css',
			[],
	...
	public static function assets() {
		...
		wp_enqueue_style(
			'searchwp-modal-form-admin-notifications',
			SEARCHWP_MODAL_FORM_PLUGIN_URL . 'assets/styles/admin/notifications.css',
			[],
			SEARCHWP_MODAL_FORM_VERSION
		);
		...
		wp_enqueue_script(
			'searchwp-modal-form-admin-notifications',
			SEARCHWP_MODAL_FORM_PLUGIN_URL . 'assets/js/admin/notifications.js',
			[],
			SEARCHWP_MODAL_FORM_VERSION
		);

		Utils::localize_script( 'searchwp-modal-form-admin-notifications' );
	}
	...
	public static function add_submenu_page( $submenu_pages ) {
		...
		$submenu_pages['notifications'] = [
			'menu_title' => esc_html__( 'Notifications', 'searchwp-modal-search-form' ) . '<span style="margin-top: 6px;" class="searchwp-admin-menu-notification-indicator"></span>',
			'menu_slug'  => SearchWPModalFormAdminMenu::MENU_SLUG . '#notifications',
			'position'   => 0,
		];
	...
	public static function output_header_button() {
		...
		?>
		<div id="swp-notifications-page-header-button" class="swp-header-menu--item swp-relative" title="<?php esc_html_e( 'Notifications', 'searchwp-modal-search-form' ); ?>">
            ...
            <div class="searchwp-notifications-notification__body">
                ...
                <div class="searchwp-notifications-notification__actions">
                    ...
                    <button type="button" class="searchwp-notification-dismiss components-button is-link" data-id="<?php echo absint( $notification['remote_id'] ); ?>">
						<?php esc_html_e( 'Dismiss', 'searchwp-modal-search-form' ); ?>
                    </button>
                </div>
            </div>
        </div>
	...
	public static function dismiss() {
		...
		if ( self::is_disabled_in_settings() ) {
			wp_send_json_error();
		}
