...
/**
 * Moyasar Invoice Gateway.
 *
 * Redirects customers to a secure Moyasar payment page (invoice) to pay with
 * card, Apple Pay, or other methods. Works with all themes and as a fallback
 * when embedded checkout is unavailable.
 *
 * @class       WC_Gateway_Moyasar_Invoice
 * @extends     WC_Payment_Gateway
 * @package     Moyasar/includes/gateways
 */
class WC_Gateway_Moyasar_Invoice extends WC_Payment_Gateway {

    use Moyasar_Gateway_Trait;

    public function __construct() {
        $this->id                 = 'moyasar_invoice';
        $this->method_title       = __( 'Moyasar Invoice', 'moyasar-payments' );
        $this->method_description = __( 'Redirect customers to a secure Moyasar payment page to pay with card, Apple Pay, or other methods. Works with all themes and as a fallback when embedded checkout is unavailable.', 'moyasar-payments' );
        ...
        $this->moyasar_icons = array( 'invoice' );

        $this->title       = Moyasar_Helper::get_moyasar_option( 'moyasar_invoice_title', __( 'Invoice', 'moyasar-payments' ) );
        $this->description = Moyasar_Helper::get_moyasar_option( 'moyasar_invoice_description', __( 'Pay via Invoice (Moyasar Payment Page).', 'moyasar-payments' ) );
    ...
    public function process_payment( $order_id ) {
        ...
        if ( $amount < 100 ) {
             wc_add_notice( __( 'The minimum amount for this payment method is 1.00 ' . $order->get_currency(), 'moyasar-payments' ), 'error' );
             return;
        }
        ...
        $back_url    = $order->get_cancel_order_url();
        ...
        $response = Moyasar_API::create_invoice( $invoice_data, $this->get_api_key() );
        ...
        if ( ! isset( $response['url'] ) ) {
            wc_add_notice( __( 'Error creating invoice. Please try again.', 'moyasar-payments' ), 'error' );
            return;
        }

        // Replace lang=en with store language
        $response['url'] = str_replace( 'lang=en', 'lang=' . get_locale(), $response['url'] );

        $order->update_meta_data( '_moyasar_invoice_id', $response['id'] );
        $order->update_meta_data( '_moyasar_invoice_url', $response['url'] );
        $order->set_transaction_id( $response['id'] );
        $order->add_order_note( sprintf( __( 'Moyasar Invoice created. Invoice ID: %s', 'moyasar-payments' ), $response['id'] ) );
