...
class Utils {
	...
	public static function get_post_type_stati( string $post_type = 'post', $engine = 'default', $skip_cache = false ) {
		...
		$post_stati = array_unique( (array) $post_stati );
	...
	/**
	 * Check if a string is valid UTF-8 encoding.
	 * Uses wp_is_valid_utf8() in WordPress 6.9+, falls back to seems_utf8() for older versions.
	 *
	 * @since 4.5.6
	 *
	 ...
	 */
	private static function is_valid_utf8( $data ) {

		if ( function_exists( 'wp_is_valid_utf8' ) ) {
			return wp_is_valid_utf8( $data );
		}

		// Fallback for WordPress < 6.9.
		return seems_utf8( $data );
	}
	...
	public static function decode_string( string $string ) {
		...
		if ( function_exists( 'mb_convert_encoding' ) && ! self::is_valid_utf8( $string ) ) {
	...
	public static function stringify_html( $html ) {
		...
		if ( function_exists( 'mb_convert_encoding' ) && ! self::is_valid_utf8( $html ) ) {
	...
	public static function maybe_decode_stringified( string $data ) {
		...
		if ( is_null( $json_decoded_input ) ) {
			// It's not JSON, but it might be serialized. Reject object instantiation —
			// callers (post/term meta indexing) only need serialized arrays of scalars,
			// and this function is reachable from unauthenticated search input.
			if ( is_serialized( $data ) ) {
				// phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged,WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize -- allowed_classes=false blocks object injection; silenced to match WP core's maybe_unserialize() behavior on malformed input.
	...
	public static function get_db_details() {
		...
		$mysql_server_type = mysqli_get_server_info( $wpdb->dbh ); // phpcs:ignore WordPress.DB.RestrictedFunctions.mysql_mysqli_get_server_info
	...
	public static function trim_string_around_substring( string $string, string $substrings, $length = 55 ) {
		$string = self::strip_shortcodes( $string );
	 ...
	 * @return false|mixed
	 */
	public static function is_highlighter_enabled( $entry, $doing_query = false, $source_name = null ) {

		if ( ! $entry instanceof Entry || empty( $source_name ) ) {
			return false;
		}
		...
		/**
		 * Filters whether highlighting is enabled.
		 *
		 * @since 4.5.2
		 *
		 * @param bool  $enabled Whether highlighting is enabled.
		 * @param Array $data    The data to highlight.
		 */
		$highlighter = apply_filters(
			"searchwp\source\\{$source_name}\do_highlighting",
			$highlighter,
			[
				'entry' => $entry,
				'query' => $doing_query,
	...
	/**
	 * Get the search terms to use for the highlighting feature.
	 *
	 * @since 4.5.2
	 *
	 * @param Query  $query       The query to get the terms for.
	 * @param String $source_name The name of the Source.
	 *
	 * @return array|bool|string|string[]
	 */
	public static function get_highlighting_terms( $query, $source_name = null ) {

		if ( ! $query instanceof Query ) {
			$search_terms = (string) $query;
		} elseif ( ! empty( $query->get_suggested_search() ) ) {
			...
			/**
			 * Filters whether the original search string should be used for highlighting.
			 *
			 * @since 4.5.2
			 *
			 * @param bool  $use_original Whether the original search string should be used for highlighting.
			 */
			$use_original_search_string = apply_filters( 'searchwp\global_excerpt\use_original_search_string', true );

			/**
			 * Filters whether the submitted search string should be used for highlighting for a specific Source.
			 *
			 * @since 4.5.2
			 *
			 * @param bool  $use_submitted Whether the submitted search string should be used for highlighting for a specific Source.
			 */
			$use_original_search_string = apply_filters( 'searchwp\source\\' . $source_name . '\global_excerpt\use_original_search_string', $use_original_search_string );

			$search_terms = $use_original_search_string
		...
		if ( false !== strpos( $search_terms, '"' ) ) {
			$search_terms = str_replace( '"', '', $query->get_keywords() );
		}
