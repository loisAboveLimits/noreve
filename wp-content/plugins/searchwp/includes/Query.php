...
class Query {
	...
	/**
	 * Getter for all query results before pagination limits.
	 *
	 * @since 4.6.0
	 *
	 * @return array
	...
	private function set_algorithm_logic_passes() {
		...
		// Add AND logic pass if we have 2+ tokens (e.g. from one keyword expanded by synonyms or split pattern)
		// or 2+ original keywords (so strict AND can return zero when only one keyword has a match).
		$should_add_and_logic = ( count( $this->tokens ) >= 2 ) || ( $original_keyword_count >= 2 );
